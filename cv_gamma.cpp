...
void parallel_cross_validation(KOIL& koil, int*& id_x, double*& y, int*& idx_j, int n_data)
{
    ...
    auto run_function = [&](int c_index, int g_index)
    {
        ...
        for(int fold=1;fold<=5;fold++){
            ...
            koil.rs_plus(id_train, n_train, id_test, n_test,
                    rs,
        ...
        auc_record[c_index][g_index] /= 5;
        acc_record[c_index][g_index] /= 5;
...
int main(int argc, char **argv)
{
    if(argc != 6)
    {
        cout<<"Argument format : "<<argv[0]<<"<dataset_file> <Num of clist> <Num of glist> <c start> <g start>"<<endl;
        return 0;
    }
